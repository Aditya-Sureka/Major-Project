...
def get_gemini_analysis(json_output, endpoint_type, vehicle_number_match, maker_match, confidence):
    try:
        ...
        # Use Gemini 1.5 Flash via new client API
        prompt = f"""
        ...
        response_text_raw = response.text if hasattr(response, "text") else ""
        logging.info(f"Gemini raw response: {response_text_raw[:500]}...")
